...
import { IImage, IImageRepository, IImageType } from "@/types/image";
 ...
 class Images {
	...
	async updateImageUser(image: IImageRepository): Promise<boolean> {
		const { bucket_key, mime_type, file_size, width, height, image_type, user_id } = image;
		const updatedImage = await pool.query(
			`UPDATE user_image SET bucket_key = $1, mime_type = $2, file_size = $3, width = $4, height = $5, image_type = $6 WHERE user_id = $7 AND image_type = $8 RETURNING *`,
			[bucket_key, mime_type, file_size, width, height, image_type, user_id, image_type]
		);
		return updatedImage.rows[0] || null;
	}
}
